...
- **Core Skill:** `yaam-memory-manager` (located at `skills/`).
- **Node Env:** Run scripts via `npx tsx skills/yaam-memory-manager/scripts/<script>.ts`.


## 2. Mandatory Workflows
- **Memory Exploration:** Before making architectural changes, query the existing relationships using:
  ```bash
  npx tsx skills/yaam-memory-manager/scripts/graph_explore.ts "<query>"
  ```
- **Task Initialization:** When starting a new task, initialize a dedicated context:
  ```bash
  npx tsx skills/yaam-memory-manager/scripts/workspace_initialize.ts --name "<your-active-task>" --description "<description>"
  ```
- **Insight Capture:** Record all "Why" decisions and learnings:
  ```bash
  npx tsx skills/yaam-memory-manager/scripts/workspace_append_note.ts --workspace "<your-active-task>" --content "<insight>"
  ```
...
```bash
npx tsx skills/yaam-memory-manager/scripts/reconciler.ts
```
